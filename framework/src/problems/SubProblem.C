...
SubProblem::preparePRefinement()
{
  ...
  for (const auto s : make_range(eq.n_systems()))
  {
    std::unordered_set<FEFamily> disable_families, enable_families;
    for (const auto & [p, flag] : _family_for_p_refinement)
    {
      auto [sysnum, family] = p;
      if (sysnum == s)
      {
        if (flag)
        {
          libmesh_assert(!enable_families.count(family));
          disable_families.insert(family);
        }
        else
        {
          libmesh_assert(!disable_families.count(family));
          enable_families.insert(family);
        }
      }

      // We may need to disable families that are used for
      // autogenerated FE objects but not variables
      for (auto family : _default_families_without_p_refinement)
      {
        if (!enable_families.count(family))
          disable_families.insert(family);
      }
    }

    if (s < numNonlinearSystems())
      for (const auto tid : make_range(libMesh::n_threads()))
        assembly(tid, s).havePRefinement(disable_families);

    auto & system = eq.get_system(s);
...
void
SubProblem::markFamilyPRefinement(unsigned int sysnum, const InputParameters & params)
{
  ...
  auto [it, inserted] = _family_for_p_refinement.emplace(std::make_pair(sysnum, family), flag);
