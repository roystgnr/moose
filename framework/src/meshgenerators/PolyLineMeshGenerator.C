...
PolyLineMeshGenerator::validParams()
{
  ...
  params.addParam<std::vector<Point>>(
      "points", "The points defining the polyline, in order");

  params.addParam<bool>(
      "loop", false, "Whether edges should form a closed loop");

  params.addParam<boundary_id_type>(
      "bcid0", 0, "Boundary id for (non-looped) polyline start");

  params.addParam<boundary_id_type>(
      "bcid1", 1, "Boundary id for (non-looped) polyline end");
