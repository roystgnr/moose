...
Poly2TriMeshGenerator::validParams()
{
  ...
  params.addParam<std::vector<BoundaryName>>("input_boundary_names",
                                             "2D-input-mesh boundaries defining the output mesh outer boundary");
  params.addParam<std::vector<SubdomainName>>("input_subdomain_names",
  ...
  params.addParam<subdomain_id_type>("output_subdomain_id", 0, "Subdomain id to set on new triangles.");
  params.addParam<SubdomainName>("output_subdomain_name", "Subdomain name to set on new triangles.");

  params.addParam<boundary_id_type>("output_boundary_id", 0, "Boundary id to set on new outer boundary.");
  params.addParam<std::vector<boundary_id_type>>("hole_boundary_ids", "Boundary ids to set on holes.  Numbered up from 1 by default.");

  params.addParam<bool>("verify_holes",
                        true,
  ...
  params.addRangeCheckedParam<Real>(
      "desired_area", std::numeric_limits<Real>::max(), "desired_area>0", "Desired (maximum) triangle area.");
...
Poly2TriMeshGenerator::generate()
{
  ...
  if (isParamValid("input_boundary_names"))
  {
    if (isParamValid("input_subdomain_names"))
      paramError("input_subdomain_names", "input_boundary_names and input_subdomain_names cannot both specify an outer boundary.");

    auto _boundary_names =
  ...
  if (isParamValid("input_subdomain_names"))
  {
    auto _subdomain_names =
      getParam<std::vector<SubdomainName>>("input_subdomain_names");

    auto _subdomain_ids =
      MooseMeshUtils::getSubdomainIDs(*mesh, _subdomain_names);
    ...
    {
      if (_subdomain_ids[i] == Moose::INVALID_BLOCK_ID ||
          !subdomains.count(_subdomain_ids[i]))
        paramError("input_subdomain_names", _subdomain_names[i], " was not found in the boundary mesh");
  ...
  {
    auto _hole_boundary_ids =
      getParam<std::vector<boundary_id_type>>("hole_boundary_ids");
    ...
    for (auto h : index_range(_hole_ptrs))
    {
      ...
      new_hole_bcid =
        std::max(new_hole_bcid,
                 boundary_id_type(_hole_boundary_ids[h]+1));
    }
  }

  if (isParamValid("output_boundary_id"))
  {
    const boundary_id_type output_boundary_id =
      getParam<boundary_id_type>("output_boundary_id");

    libMesh::MeshTools::Modification::change_boundary_id
      (*mesh, end_bcid, output_boundary_id);

    new_hole_bcid = std::max(new_hole_bcid,
                             boundary_id_type(output_boundary_id+1));
  }
