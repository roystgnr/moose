...
Poly2TriMeshGenerator::validParams()
{
  ...
  params.addParam<bool>("smooth_triangulation", false, "Whether to do Laplacian mesh smoothing on the generated triangles.");
  ...
  for (auto hole_i : index_range(_hole_ptrs))
    {
      if (hole_i < _stitch_holes.size() &&
          _stitch_holes[hole_i])
        {
          ...
          const std::set<boundary_id_type> & local_hole_bcids =
            hole_boundary_info.get_boundary_ids();
          ...
          mooseAssert(found_hole_sides == np,
